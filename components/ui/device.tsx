...
/**
 * Apple device bezels, adapted from the `Device` component in
 * aliimam-in/templates (apps/template-02/components/ui/device.tsx).
 *
 * The original painted the screen with an SVG `<image href>`, which only
 * accepts images. To host arbitrary content — video, live DOM — the artwork is
 * split into two layers instead: `back` (body, bezel, screen fill) renders in
 * an SVG below, `front` (the few bits that sit over the glass, e.g. the
 * MacBook notch) renders in an SVG above, and `children` go in a plain HTML
 * box positioned between them at the screen rectangle. That keeps the
 * stacking order of the original while letting the screen hold anything.
 *
 * Screen geometry is declared in the SVG's own user units and converted to
 * percentages at render time, so a device scales fluidly with its container.
 */
...
interface ScreenRect {
  ...
  width: number;
  height: number;
  /** Corner radius in user units; converted to an elliptical CSS radius. */
  radius: number;
}
...
interface DeviceArtwork {
  ...
  screen: ScreenRect;
  back: ReactNode;
  /** Artwork that overlaps the screen and must paint above its content. */
  front?: ReactNode;
}

type DeviceVariant = "macbook" | "imac" | "iphone" | "ipad";

interface DeviceProps extends Omit<SVGProps<SVGSVGElement>, "children"> {
  variant?: DeviceVariant;
  /** Convenience for a static screenshot; ignored when `children` is given. */
  src?: string;
  /** Rendered into the screen, clipped to its rounded rectangle. */
  children?: ReactNode;
  className?: string;
  screenClassName?: string;
}

const MACBOOK: DeviceArtwork = {
  width: 650,
  height: 400,
  screen: { x: 74.5, y: 21.5, width: 501, height: 323, radius: 5 },
  back: (
    <>
      <path
        fill="#a4a5a7"
...
const IMAC: DeviceArtwork = {
  width: 600,
  height: 500,
  screen: { x: 29.12, y: 25.02, width: 541.76, height: 305.06, radius: 0.44 },
  back: (
    <>
      <rect fill="url(#device-imac-stand)" x="232.4" y="401.32" width="135.19" height="83.37" />
      ...
      <defs>
        <linearGradient id="device-imac-stand" x1="300" y1="484.69" x2="300" y2="401.32" gradientUnits="userSpaceOnUse">
...
const IPHONE: DeviceArtwork = {
  width: 200,
  height: 400,
  screen: { x: 12.7, y: 10.11, width: 174.62, height: 379.78, radius: 26.97 },
  back: (
    <>
      <path
        fill="#303333"
      ...
      />
      <rect fill="currentColor" x="12.7" y="10.11" width="174.62" height="379.78" rx="26.97" ry="26.97" />
    </>
  ),
  front: (
    <>
      ...
      />
      <path fill="#080d4c" d="M118.78,27.68c-4.32.06-4.32-6.73,0-6.66,4.32-.06,4.32,6.73,0,6.66Z" />
    </>
  ),
};

const IPAD: DeviceArtwork = {
  width: 520,
  height: 400,
  screen: { x: 31.37, y: 28.47, width: 457.25, height: 342.87, radius: 9.61 },
  back: (
    <>
      <path
        fill="#aaabac"
...
const ARTWORK: Record<DeviceVariant, DeviceArtwork> = {
  macbook: MACBOOK,
  imac: IMAC,
  iphone: IPHONE,
  ipad: IPAD,
...
function Device({ variant = "macbook", src, children, className, screenClassName, ...props }: DeviceProps) {
  const { width, height, screen, back, front } = ARTWORK[variant];
  const viewBox = `0 0 ${width} ${height}`;
  const percent = (value: number, basis: number) => `${(value / basis) * 100}%`;

  return (
    <div data-slot="device" data-variant={variant} className={cn("relative", className)}>
      <svg viewBox={viewBox} fill="none" xmlns="http://www.w3.org/2000/svg" className="block w-full" {...props}>
        {back}
      </svg>
      <div
        ...
        style={{
          left: percent(screen.x, width),
          top: percent(screen.y, height),
          width: percent(screen.width, width),
          height: percent(screen.height, height),
          // Elliptical radius: the screen's aspect differs from the corner's,
          // so a single percentage would skew the curve.
          borderRadius: `${percent(screen.radius, screen.width)} / ${percent(screen.radius, screen.height)}`,
        }}
      >
        {children ??
          (src ? (
            // Plain <img> rather than next/image so the component stays
            // framework-agnostic for registry consumers. Pass an optimized
            // <Image /> as children when you want one.
...
export type { DeviceProps, DeviceVariant };
export { Device };
